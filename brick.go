...
the variable valuePtr.  RecvValue does not contain a for {} loop; it terminates once 
the value has been read from the in channel
See its use in PlusP
*/
...
it send all received values on the in channel to the out channel unaltered.
*/
...
and it then sends the difference of the values to the out channel.  
...
A value has to be received from every input channel before any output is generated.
It would be nice to do this concurrently but we have seen in PlusP that this
